(function() {
  ...
  $isa = {
    ...
    primitive: function(x) {
      return $primitive_types.has($type_of(x));
    }
  };

  //-----------------------------------------------------------------------------------------------------------
  $primitive_types = new Set(['null', 'undefined', 'infinity', 'boolean', 'nan', 'float', 'anyfloat', 'text']);
  ...
  $type_of = function(x) {
    ...
    switch (jstypeof = typeof x) {
      case 'number':
        return 'anyfloat'/* depends on `infinity`, `nan`, `float`, `anmyfloat` */;
      case 'string':
        return 'text';
    }
    if (Array.isArray(x)) {
    ...
    switch (millertype = Object.prototype.toString.call(x)) {
