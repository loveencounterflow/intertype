(function() {
  'use strict';
  var E, GUY, Intertype_abc, debug, help, info, misfit, notavalue, rpr, to_width, urge, warn,
    indexOf = [].indexOf;
...
/* Mark Miller Device */      return 'wrapper';
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.get_state_report = function(hub) {
    var R, dent, error, hedge, hedgeresults, hedgerow, hedges, i, len, level, line, r, ref, result, truth, value, verb, x, xr;
    // ( verb, hedgerow, value, types ) ->
    truth = function(b, r) {
      return GUY.trm.reverse(b ? GUY.trm.green(" T ") : GUY.trm.red(" F "));
    };
    // hedges  = hub.state.hedgerow.split '.'
    // console.log '^get_state_report@2323^', hub.state
    ({hedges, hedgerow, hedgeresults, x, result, verb, error} = hub.state);
    R = [];
    xr = ` ${rpr(x)} `;
    ...
    R.push(xr);
    R.push('\n');
    for (i = 0, len = hedgeresults.length; i < len; i++) {
      [ref, level, hedge, value, r] = hedgeresults[i];
    ...
    if (error instanceof Error) {
      R.push(GUY.trm.red(GUY.trm.reverse(` Error: ${error.message.trim()} \n`)));
    } else if (error != null) {
      R.push(GUY.trm.red(GUY.trm.reverse(` Error: ${error.toString()} \n`)));
    }
    if ((verb === 'validate') && (result !== false)) {
      //.........................................................................................................
      result = true;
    }
    R.push(truth(result, result != null ? result.toString() : void 0));
    //.........................................................................................................
    return R.join('');
  };
