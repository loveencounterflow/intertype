(function() {
  'use strict';
  var E, GUY, H, HEDGES, ITYP, Intertype_abc, debug, deep_copy, equals, help, rpr, to_width, types, urge, warn,
    splice = [].splice;
  ...
  this.Type_cfg = class Type_cfg extends Intertype_abc {
    //---------------------------------------------------------------------------------------------------------
    constructor(hub, cfg) {
      var f, k, keys, self, test, tests, v;
  ...
  this.Intertype = (function() {
    class Intertype extends Intertype_abc {
      ...
      _get_hedge_base_proxy_cfg(self, method_name) {
        return {
          ...
          get: (target, key) => {
            ...
            GUY.props.hide(self, key, R = new Proxy(f, this._get_hedge_sub_proxy_cfg(self)));
            return R;
          }
        };
      }
      ...
      //---------------------------------------------------------------------------------------------------------
      _isa(...hedges) {
        var R, e, hedge, hedge_idx, last_hedge_idx, ref, tail_hedges, type, type_cfg, verdict, x;
      ...
      //---------------------------------------------------------------------------------------------------------
      _test_hedge(hedge, x) {
        var R, hedgetest, ref, type_cfg;
        debug('^3345^', {hedge}, (ref = GUY.props.get(this.registry, hedge, null)) != null ? ref.test : void 0);
