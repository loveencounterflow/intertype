  ...
  Intertype = (function() {
    ...
    class Intertype extends H.Intertype_abc {
      ...
      _declare(...P) {
        ...
        this.isa[dsc.name] = new Proxy(dsc, this._get_hedge_sub_proxy_cfg(this));
        this.validate[dsc.name] = new Proxy(((x) => {
          return this._validate(dsc.name, x);
        }), this._get_hedge_sub_proxy_cfg(this));
        if (dsc.collection) {
          this._collections.add(dsc.name);
        }
        return null;
      }
