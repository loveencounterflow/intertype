      ...
      _get_hedge_sub_proxy_cfg(self) {
        return {
          get: (target, key) => {
            ...
            if (key === 'of') {
              if (!self.groups.collection.has(target.name)) {
                throw new E.Intertype_ETEMPTBD('^intertype.isa@2^', `expected type before \`of\` to be a collection, got ${rpr(target.name)}`);
              }
            }
            //...................................................................................................
            f = {
      ...
      _isa(...hedges) {
        ...
        while (true) {
          ...
          switch (hedge) {
            ...
            case 'of':
              if (hedge_idx === 0) {
                throw new E.Intertype_ETEMPTBD('^intertype.isa@1^', "hedgerow cannot start with `of`, must be preceded by hedge");
              }
              if (hedge_idx === last_hedge_idx) {
                throw new E.Intertype_ETEMPTBD('^intertype.isa@2^', "hedgerow cannot end with `of`, must be succeeded by hedge");
              }
              // element_mode = true
