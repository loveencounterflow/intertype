  ...
  this.Intertype = (function() {
    class Intertype extends Intertype_abc {
      ...
      constructor(cfg) {
        ...
        GUY.props.hide(this, 'validate', new Proxy(this._validate.bind(this), {
          get: (_, type) => {
            return (cfg) => {
              return this._validate.call(this, type, cfg);
            };
          }
        }));
        ...
        return new Proxy(this, {
          get: ((t, k) => {
            debug(GUY.trm.reverse(GUY.trm.steel('^323————————————————————————————————————^', rpr(k))));
            return t[k];
          })
        });
      }
...
// debug '^3324^', { tail_hedges, }
      ...
      _protocol_isa(term, result, verdict) {
        var test;
        // debug GUY.trm.gold '^_protocol_isa@1^', { term, result, verdict, }
        test = GUY.props.get(this.registry, term, null);
        debug(GUY.trm.gold('^_protocol_isa@1^', {term, verdict, test}));
        return verdict;
      }
