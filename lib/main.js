(function() {
  'use strict';
  var CND, Defaults, E, Empty, GUY, ITYP, Intertype_abc, Isa, Isa_empty, Isa_list_of, Isa_nonempty, Isa_optional, List_of, Nonempty, Validate, Validate_empty, Validate_list_of, Validate_nonempty, Validate_optional, alert, badge, debug, echo, empty, error, help, info, js_type_of, length_of, list_of, log, njs_path, nonempty, praise, ref, rpr, urge, warn, whisper, x,
  ...
  ref = this.Intertype = (function() {
    class Intertype extends Intertype_abc {
      ...
      * _XXX_walk_permutations(type_cfg, hedge_idx = 0, current_path = [], R = []) {
        var hedge, i, len, next_path, ref1, term, term_idx;
        /* thx to https://itecnote.com/tecnote/java-generate-all-combinations-from-multiple-lists/ */
        if (hedge_idx === this.constructor.hedges.length) {
          yield current_path;
          return null;
        }
        // R.push current_path
        // return R
        hedge = this.constructor.hedges[hedge_idx];
        yield* this._XXX_walk_permutations(type_cfg, hedge_idx + 1, current_path, R);
        ref1 = hedge.x;
        for (term_idx = i = 0, len = ref1.length; i < len; term_idx = ++i) {
          term = ref1[term_idx];
          next_path = [...current_path];
          next_path.push(term);
          yield* this._XXX_walk_permutations(type_cfg, hedge_idx + 1, next_path, R);
        }
        return null;
      }
      ...
      declare(...hedges) {
        ...
        var mandatory_empty_name, mandatory_empty_test, mandatory_name, mandatory_nonempty_name, mandatory_nonempty_test, mandatory_test, optional_empty_name, optional_empty_test, optional_name, optional_nonempty_name, optional_nonempty_test, optional_test, ref1, test, type, type_cfg;
        boundMethodCheck(this, ref);
        ref1 = hedges, [...hedges] = ref1, [type, type_cfg] = splice.call(hedges, -2);
        type_cfg = new ITYP.Type_cfg(type_cfg);
        hedges.push(type);
        if (hedges[0] === 'optional') {
          throw new E.Intertype_ETEMPTBD('^intertype@1^', `'optional' cannot be a hedge in declarations, got ${rpr(hedges)}`);
        }
        mandatory_name = hedges.join(this.cfg.sep);
        mandatory_test = type_cfg.test.bind(this);
        optional_name = `optional${this.cfg.sep}${mandatory_name}`;
        optional_test = (test = function(x) {
          return (x == null) || (mandatory_test(x));
        }).bind(this);
        // debug '^4234^', { mandatory_name, mandatory_test, optional_name, optional_test, }
        this._types[mandatory_name] = {
          ...type_cfg,
          name: mandatory_name,
          type,
          test: mandatory_test
        };
        this._types[optional_name] = {
          ...type_cfg,
          name: optional_name,
          type,
          test: optional_test
        ...
        if (type_cfg.isa_collection) {
          mandatory_empty_name = `empty${this.cfg.sep}${mandatory_name}`;
          mandatory_empty_test = (test = function(x) {
            return (mandatory_test(x)) && (this._is_empty(type_cfg, x));
          }).bind(this);
          optional_empty_name = `optional${this.cfg.sep}${mandatory_empty_name}`;
          optional_empty_test = (test = function(x) {
            return (x == null) || (mandatory_empty_test(x));
          }).bind(this);
          this._types[mandatory_empty_name] = {
            ...type_cfg,
            name: mandatory_empty_name,
            type,
            test: mandatory_empty_test
          };
          this._types[optional_empty_name] = {
            ...type_cfg,
            name: optional_empty_name,
            type,
            test: optional_empty_test
          };
          mandatory_nonempty_name = `nonempty${this.cfg.sep}${mandatory_name}`;
          mandatory_nonempty_test = (test = function(x) {
            return (mandatory_test(x)) && (this._is_nonempty(type_cfg, x));
          }).bind(this);
          optional_nonempty_name = `optional${this.cfg.sep}${mandatory_nonempty_name}`;
          optional_nonempty_test = (test = function(x) {
            return (x == null) || (mandatory_nonempty_test(x));
          }).bind(this);
          this._types[mandatory_nonempty_name] = {
            ...type_cfg,
            name: mandatory_nonempty_name,
            type,
            test: mandatory_nonempty_test
          };
          this._types[optional_nonempty_name] = {
            ...type_cfg,
            name: optional_nonempty_name,
            type,
            test: optional_nonempty_test
          };
        }
        return null;
      }
      ...
      //---------------------------------------------------------------------------------------------------------
      _is_empty(type_cfg, x) {
        return (this._size_of(type_cfg, x)) === 0;
      }

      _is_nonempty(type_cfg, x) {
        return (this._size_of(type_cfg, x)) > 0;
      }
...
/* Mark Miller Device */          return 'wrapper';
    ...
    //---------------------------------------------------------------------------------------------------------
    Intertype.hedges = GUY.lft.freeze([
      {
        x: ['optional'],
        match: {
          all: true
