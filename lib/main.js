(function() {
  'use strict';
  var DECLARATIONS, E, GUY, H, HEDGES, ITYP, Intertype_abc, debug, deep_copy, equals, help, nameit, rpr, to_width, types, urge, warn,
    splice = [].splice;
  ...
  //-----------------------------------------------------------------------------------------------------------
  types.declare('Type_cfg_constructor_cfg_NG', {
    tests: {
      "@isa.object x": function(x) {
        return this.isa.object(x);
      },
        ...
        return {
          ...
          get: (target, key) => {
            ...
            if (method_name === '_create') {
              f = nameit(key, function(cfg = null) {
                return self[self.state.method](key, cfg);
              });
            } else {
              f = nameit(key, function(...P) {
